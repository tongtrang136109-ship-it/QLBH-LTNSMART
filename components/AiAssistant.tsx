...
// Chuyển File/Blob -> base64 (bỏ prefix data:)
const blobToBase64 = (blob: Blob): Promise<string> =>
  new Promise((resolve, reject) => {
    const reader = new FileReader()
    reader.onloadend = () => {
      if (typeof reader.result === "string") {
        const base64 = reader.result.split(",")[1] // cắt "data:image/png;base64,"
        resolve(base64)
      } else {
        reject(new Error("Không đọc được ảnh dưới dạng base64"))
      }
    }
...
const AiAssistant: React.FC = () => {
  ...
  const handleDiagnose = async () => {
    ...
    try {
      ...
      setResponse(result || "Không có nội dung trả về.")
    } catch (e: any) {
      console.error(e)
      setError(e?.message || "Không thể nhận được chẩn đoán. Vui lòng thử lại.")
    } finally {
      setIsLoading(false)
    }
  }

  // Parse text có tiêu đề dạng **Tiêu đề:**
  const parseAndFormatResponse = (text: string) => {
    ...
    return sections.map((s, i) => (
      <details
        key={i}
        open
        className="mb-4 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg overflow-hidden last:mb-0"
      >
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex items-center">
        ...
        <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-100 ml-3">
          Trợ lý Chẩn đoán AI
        </h1>
      </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="flex items-center gap-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold py-2 px-4 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
            >
              <CloudArrowUpIcon className="w-5 h-5" /> Tải ảnh lên
            </button>
            <button
              className="flex items-center gap-2 bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold py-2 px-4 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors disabled:opacity-50"
              disabled
            >
          ...
          {image && (
            <div className="mt-4 relative w-40 h-40">
              <img
                ...
                alt="Preview"
                className="w-full h-full object-cover rounded-lg border-2 border-slate-200 dark:border-slate-600"
              />
              <button
      ...
      {response && !isLoading && (
        <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-xl border border-slate-200/60 dark:border-slate-700">
          ...
          <div>{parseAndFormatResponse(response)}</div>
        </div>
      )}
    </div>
  )
