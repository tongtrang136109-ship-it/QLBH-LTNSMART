            ...
            <div className="bg-white dark:bg-slate-900 rounded-lg shadow-xl w-full max-w-sm">
                ...
                <div className="p-6 space-y-4">
                    <div>
                        ...
                        <input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} className="mt-1 w-full p-2 border dark:border-slate-600 rounded-md dark:bg-slate-800 dark:text-white" autoFocus />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Xác nhận mật khẩu mới</label>
                        <input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="mt-1 w-full p-2 border dark:border-slate-600 rounded-md dark:bg-slate-800 dark:text-white" />
                    </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[60] flex justify-center items-center p-4">
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-md">
                <div className="p-4 border-b dark:border-slate-700 flex justify-between items-center">
                    <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">Chọn dữ liệu hiển thị</h3>
                ...
                <div className="bg-slate-50 dark:bg-slate-700/50 px-6 py-3 flex justify-between items-center border-t dark:border-slate-700">
                    <div>
                        <button onClick={() => setSelection(DEFAULT_VISIBLE_COLUMNS)} className="text-sm text-sky-600 dark:text-sky-400 hover:underline">Đặt mặc định</button>
                        <button onClick={() => setSelection([])} className="text-sm text-red-600 dark:text-red-400 hover:underline ml-4">Xoá tuỳ chọn</button>
                    </div>
...
const PermissionRow: React.FC<{
    ...
    if (config.type === 'toggle') {
        return (
            <div className="flex justify-between items-center p-3 border dark:border-slate-700 rounded-md bg-white dark:bg-slate-800">
                <span className="font-medium text-slate-700 dark:text-slate-200">{config.name}</span>
                <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" checked={!!value} onChange={(e) => onChange(e.target.checked)} className="sr-only peer" />
                    <div className="w-11 h-6 bg-gray-200 rounded-full peer peer-focus:ring-2 peer-focus:ring-sky-300 dark:peer-focus:ring-sky-800 dark:bg-slate-600 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-sky-600"></div>
                </label>
            </div>
        );
    }
    ...
    return (
        <div className="border dark:border-slate-700 rounded-md bg-white dark:bg-slate-800">
            <div className="grid grid-cols-1 md:grid-cols-4 items-center p-3 gap-2">
                <div className="md:col-span-1 font-medium text-slate-700 dark:text-slate-200">{config.name}</div>
                <div className="md:col-span-3 grid grid-cols-3 gap-2 text-slate-700 dark:text-slate-300">
                    {(['all', 'restricted', 'none'] as PermissionLevel[]).map(l => (
                        <label key={l} className="flex items-center space-x-2 cursor-pointer p-2 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700">
            ...
            {level === 'restricted' && (
                <div className="p-3 border-t dark:border-slate-700 bg-slate-50 dark:bg-slate-700 text-slate-700 dark:text-slate-300">
                    <button type="button" onClick={() => setIsExpanded(!isExpanded)} className="text-sm text-sky-600 dark:text-sky-400 hover:underline mb-2">
...
const PermissionsModal: React.FC<{
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[70] flex justify-center items-center p-4">
            <div className="bg-slate-100 dark:bg-slate-900 rounded-lg shadow-xl w-full max-w-4xl h-[90vh] flex flex-col">
                <div className="p-4 border-b dark:border-slate-700 bg-white dark:bg-slate-800 rounded-t-lg flex justify-between items-center">
                    <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100">{department.id ? 'Chỉnh sửa thông tin' : 'Thêm phòng ban'}</h3>
                    <button onClick={onClose}><XMarkIcon className="w-6 h-6 text-slate-500 dark:text-slate-300" /></button>
                </div>
                <div className="p-6 flex-1 overflow-y-auto space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white dark:bg-slate-800 p-4 rounded-lg border dark:border-slate-700">
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Tên (*)</label>
                            <input type="text" value={data.name || ''} onChange={e => setData(d => ({...d, name: e.target.value}))} className="mt-1 w-full p-2 border dark:border-slate-600 rounded-md dark:bg-slate-700 dark:text-white" />
                        </div>
                         <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Mô tả</label>
                            <input type="text" value={data.description || ''} onChange={e => setData(d => ({...d, description: e.target.value}))} className="mt-1 w-full p-2 border dark:border-slate-600 rounded-md dark:bg-slate-700 dark:text-white" />
                        </div>
                    </div>
                    <div className="space-y-3">
                        <h4 className="font-semibold text-slate-800 dark:text-slate-100 text-lg">Phân quyền</h4>
                ...
                <div className="bg-white dark:bg-slate-800 px-6 py-3 flex justify-end space-x-3 border-t dark:border-slate-700 rounded-b-lg">
                    <button onClick={onClose} className="bg-slate-200 text-slate-800 dark:bg-slate-700 dark:text-slate-200 font-semibold py-2 px-4 rounded-lg hover:bg-slate-300 dark:hover:bg-slate-600">Hủy</button>
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[60] flex justify-center items-center p-4">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                <div className="p-4 border-b dark:border-slate-700 flex justify-between items-center">
                    <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100">Danh sách phòng ban</h3>
                    <button onClick={onClose}><XMarkIcon className="w-6 h-6 text-slate-500 dark:text-slate-300" /></button>
                </div>
                <div className="p-6 flex-1 overflow-y-auto space-y-3">
                    {departments.map(dept => (
                        <div key={dept.id} className="p-3 border dark:border-slate-700 rounded-lg flex justify-between items-center hover:bg-slate-50 dark:hover:bg-slate-700">
                            <div>
                            ...
                            <div className="flex items-center space-x-2">
                                <button onClick={() => setEditingDepartment(dept)} className="p-2 text-sky-600 dark:text-sky-400 hover:text-sky-800 dark:hover:text-sky-300"><PencilSquareIcon className="w-5 h-5"/></button>
                                <button onClick={() => onDelete(dept.id)} className="p-2 text-red-500 hover:text-red-700"><TrashIcon className="w-5 h-5"/></button>
                            </div>
                        </div>
                    ))}
                </div>
                <div className="bg-slate-50 dark:bg-slate-700/50 px-6 py-3 border-t dark:border-slate-700 flex justify-end">
...
const UserModal: React.FC<{
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-[60] flex justify-center items-center p-4">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-xl max-h-[90vh] flex flex-col">
                <div className="p-4 border-b dark:border-slate-700 bg-orange-500 rounded-t-lg text-white flex justify-between items-center">
                ...
                <div className="flex p-4 border-b dark:border-slate-700">
                    <div className="flex items-center">
                        ...
                        <span className={`ml-2 font-semibold ${step >= 1 ? 'text-sky-600 dark:text-sky-400' : 'text-slate-500 dark:text-slate-400'}`}>Điện thoại đăng nhập</span>
                    </div>
                    <div className="flex-1 border-t-2 mx-4 mt-3 border-dashed"></div>
                     <div className="flex items-center">
                        <CheckCircleIcon className={`w-6 h-6 ${step === 2 ? 'text-sky-600 dark:text-sky-400' : 'text-slate-400 dark:text-slate-500'}`}/>
                        <span className={`ml-2 font-semibold ${step === 2 ? 'text-sky-600 dark:text-sky-400' : 'text-slate-500 dark:text-slate-400'}`}>Thêm thành viên</span>
                    </div>
                </div>
                ...
                <div className="p-6 flex-1 overflow-y-auto">
                    {step === 1 && (
                        <div className="flex flex-col items-center justify-center h-full">
                            <label className="text-lg font-semibold mb-4 dark:text-slate-200">Số điện thoại đăng nhập</label>
                            <input 
                                type="text" 
                                value={loginPhone} 
                                onChange={e => setLoginPhone(e.target.value)} 
                                className="w-full max-w-sm p-3 text-center border-2 dark:bg-slate-700 dark:border-slate-600 dark:text-white rounded-lg text-lg focus:ring-sky-500 focus:border-sky-500" 
                    ...
                    {step === 2 && (
                        <div className="space-y-4">
                             <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Số điện thoại đăng nhập (*)</label>
                                <input type="text" value={formData.loginPhone || ''} onChange={e => setFormData(d => ({...d, loginPhone: e.target.value}))} className="mt-1 w-full p-2 border dark:border-slate-600 rounded-md dark:bg-slate-700 dark:text-white" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Tên (*)</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Phòng ban</label>
                                <div className="mt-2 space-y-2 p-3 border dark:border-slate-600 rounded-md max-h-40 overflow-y-auto dark:text-slate-200">
                                    {departments.map(dept => (
                ...
                {step === 2 && (
                    <div className="bg-slate-50 dark:bg-slate-700 px-6 py-3 border-t dark:border-slate-600 flex justify-between items-center">
                        <button onClick={() => setStep(1)} className="flex items-center gap-2 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100"><ArrowUturnLeftIcon className="w-5 h-5"/> Quay lại</button>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-sm border border-slate-200/60 dark:border-slate-700 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                <input type="text" placeholder="Tìm kiếm nhân viên..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full sm:w-72 p-2 border dark:border-slate-600 rounded-md dark:bg-slate-700 dark:text-slate-100"/>
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <button onClick={() => setIsColumnSelectorOpen(true)} className="p-2 bg-slate-100 dark:bg-slate-700 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600" title="Chọn cột hiển thị"><TableCellsIcon className="w-6 h-6 text-slate-700 dark:text-slate-200"/></button>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200/60 dark:border-slate-700 overflow-x-auto">
                 <table className="w-full text-left min-w-max">
                    <thead className="border-b dark:border-slate-700 bg-slate-50 dark:bg-slate-700/50">
                        <tr>
                            {visibleColumns.map(key => {
                                ...
                                return <th key={key} className="p-3 font-semibold text-slate-600 dark:text-slate-300">{col?.name}</th>
                            })}
                            <th className="p-3 font-semibold text-slate-600 dark:text-slate-300"></th>
                        </tr>
                    </thead>
                    <tbody>
                        {paginatedUsers.map((user, index) => (
                            <tr key={user.id} className="border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                {visibleColumns.map(key => (
                                    <td key={key} className="p-3 text-slate-800 dark:text-slate-200">
                                        {(() => {
                                            switch (key) {
                                                case 'stt': return (currentPage - 1) * ITEMS_PER_PAGE + index + 1;
                                                case 'name': return <span className="font-semibold">{user.name}</span>;
                                                case 'status': return <span className={`px-2 py-1 text-xs font-semibold rounded-full ${user.status === 'active' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300'}`}>{user.status === 'active' ? 'Hoạt động' : 'Bị khoá'}</span>;
                                ...
                                <td className="p-3">
                                     <div className="flex items-center gap-2">
                                        <button onClick={() => { setEditingUser(user); setIsUserModalOpen(true); }} className="p-1 text-sky-600 dark:text-sky-400 hover:text-sky-800 dark:hover:text-sky-300"><PencilSquareIcon className="w-5 h-5"/></button>
                  ...
                  {filteredUsers.length === 0 && <div className="text-center p-8 text-slate-500 dark:text-slate-400">Không tìm thấy nhân viên nào.</div>}
            </div>
